...
void BPLUSTREE_TYPE::Remove(const KeyType &key) {
  ...
  B_PLUS_TREE_LEAF_PAGE_TYPE *leaf_node = reinterpret_cast<B_PLUS_TREE_LEAF_PAGE_TYPE *>(page->GetData());
...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node) {
  ...
  N *sibling;
  bool ret = FindSibling(node, sibling);
  Page *parent_page = buffer_pool_manager_->FetchPage(node->GetParentPageId());
  B_PLUS_TREE_INTERNAL_PAGE *parent = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE *>(parent_page->GetData());
  if(node->GetSize() + sibling->GetSize() <= node->GetMaxSize()) {
    if(ret) {
      Coalesce(&node, &sibling, &parent, parent->ValueIndex(sibling->GetPageId()));
    }
    else {
      Coalesce(&sibling, &node, &parent, parent->ValueIndex(node->GetPageId()));
    }
    buffer_pool_manager_->UnpinPage(parent->GetPageId(), true);
    return true;
  }
  else {
    Redistribute(sibling, node, parent->ValueIndex(node->GetPageId()));
...
bool BPLUSTREE_TYPE::FindSibling(N *node, N *&sibling) {
  ...
  int siblingIndex = index==0 ? index+1: index-1;
